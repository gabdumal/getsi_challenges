#ifndef __PREFERENTIAL_QUEUES_HPP__
#define __PREFERENTIAL_QUEUES_HPP__
    ...
    public:
        static vector<Client> parseInputToClients(string input) {
            try {
                ...
                size_t quantity = stoul(quantity_string);
        ...
        static vector<Client> getClientsThatArrivedAt(int minute, vector<Client> all_clients);

        static void enqueueClient(Client client, Queue<Client> &regular, Queue<Client> &preferential, Queue<Client> &urgent);

        static bool areThereAnyClientsToDequeue(Queue<Client> &regular, Queue<Client> &preferential, Queue<Client> &urgent) {
            return !urgent.isEmpty() || !preferential.isEmpty() || !regular.isEmpty();
        }

        static Client dequeueClient(Queue<Client> &regular, Queue<Client> &preferential, Queue<Client> &urgent);
...
#endif  // __PREFERENTIAL_QUEUES_HPP__
