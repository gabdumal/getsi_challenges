#ifndef __CINEPHILES_ON_VACATIONS_HPP__
#define __CINEPHILES_ON_VACATIONS_HPP__
    ...
    public:
        ...
        static string printMovies(UnorderedMap<string, Movie> movies) {
            ...
            for (auto movie_pair : movies) {
                ...
                float plot_avg = (float) movie.plot_sum / (float) movie.quantity;
                float acting_avg = (float) movie.acting_sum / (float) movie.quantity;
                float direction_avg = (float) movie.direction_sum / (float) movie.quantity;
                float soundtrack_avg = (float) movie.soundtrack_sum / (float) movie.quantity;
                float special_effects_avg = (float) movie.special_effects_sum / (float) movie.quantity;
...
#endif  // __CINEPHILES_ON_VACATIONS_HPP__
